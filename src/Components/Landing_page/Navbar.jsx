...
const Navbar = () => {
  ...
  const renderNavLink = (link, useMagnet = true) => {
    ...
    let isActive = false;
    if (link.isRouter) {
      // Only match pathname for router links
  ...
  return (
    <nav className={`flex flex-wrap justify-between items-center p-5 sticky top-0 z-50 
      ${isDarkMode ? 'bg-gray-900/40 backdrop-blur-lg text-white' : 'bg-[#F4F7FF]/30 backdrop-blur-lg text-black'} 
